...
> Most common effects studied:
  ...
  2. Mortality (1493)
  3. Behavior (360)
> The researchers are interested in these effects because they are measures of how the insecticides impact insects. Population abundance and mortality demonstrate how insecticides impact insect viability while insect behavior could include important life history activities like feeding and mating.
...
> Answer: The class of "Conc.1..Author" is "factor." When loading the dataset, we told R to read strings as factors. (??????????????)
...
##create another dataframe filtering by year and month
Litter_Aug_2018 <- 
  Litter %>%
  mutate(year=year(collectDate_date)) %>%
  mutate(month=month(collectDate_date)) %>%
  filter(year == "2018") %>%
  filter(month == "8")
View(Litter_Aug_2018)

##using unique function to determine sample days
unique(Litter_Aug_2018$collectDate_date)
...
> Answer: The violin plot just shows flat lines, so the boxplot shows more information about the data (??????????????)
