...
> Answer:
H0: slope = 0 (there is no significant relationship between lake temperature and depth)
Ha: slope =/= 0 (lake temperature varies significantly with water depth)
...
> Answer: The figure demonstrates that temperature generally decreases with increasing water depth. The trend does not seem entirely linear in that the temperature seems to level off at about 10 meters.... (greater range of temps at lower depths)?????????????????????????????
...
> Answer: The model selected by AIC model selection includes year4, daynum, and depth. The R-squared value of this model is 0.7412, meaning about 74% of the variation in temperature is explained by changes in year, day of the year, and water depth. This model is improved over the depth model as the R-squared value is a bit higher. 
...
> Answer: Yes, there is a significant difference in temperatures between the different lakes as the p-value of the ANOVA test was less than 2e-16. The linear model explains about 4% of variation in temperature. 
...
lake_temp_groups <- HSD.test(temp_aov, "lakename", group = T)
lake_temp_groups
##should we just use the Tukey HSD test and say which groups of two lakes have the same or find the groups?
...
>Answer: looks like from the TukeyHSD test that the p-value has been adjusted for multiple comparisons
