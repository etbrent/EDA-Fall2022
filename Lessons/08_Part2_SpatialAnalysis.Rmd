...
HUC <- st_read('./Data/Spatial/NCHUC8.shp') 
...
HUC2 <- st_read('./Data/Spatial/NCHUC8.shp') %>%
  filter(SUBBASIN == "Upper Neuse")

#Create a map
mapview(HUC2, col.regions="purple") + 
  mapview(filter(HUC, !SUBBASIN == "Upper Neuse"), col.regions="orange")

mapview(HUC, col.regions="orange") +
mapview(HUC2, col.regions="purple", alpha.regions=1) 
...
mapview(nc_powerplants_sf, zcol="PRIM_FUEL")
...
wind_energy <- read.csv(
  ...
  group_by(HUC_8) %>%
  summarize(sum_kWh = sum(AvgWindEnergy))


#Join to HUC_8 features
HUC8_join <- HUC %>% 
  left_join(wind_energy, by = "HUC_8") 


#View the outputs
mapview(HUC8_join, zcol="sum_kWh")
...
mapview(HUC_aggregate, zcol="acres")

#Map the data
ggplot() + geom_sf(data=HUC_aggregate,aes(fill=acres))
...
huc8_sf_utm <-     st_transform(HUC, crs = 26917)
huc2_utm <-        st_transform(HUC_aggregate, crs = 26917)
```
...
triangle_counties <- counties_sf_utm %>%
  filter(NAME == "Chatham" | NAME == "Durham" | NAME == "Orange" | NAME =="Wake")

#Grab the intersecting HUC_8s
#triangle_intersect_2 <- huc8_sf_utm %>% 
 # filter(st_intersects(x = ., y = triangle_counties, sparse = FALSE)) ##don't know what to do

triangle_intersect <- huc8_sf_utm[triangle_counties,]

mapview(triangle_counties) +
  mapview(triangle_intersect, alpha.regions = 0, color="red")


#Intersect the HUC_8s
triangle_intersect2 <- huc2_utm %>% 
  st_intersection(triangle_counties) ##??????

mapview (triangle_counties)+
  mapview(triangle_intersect2, alpha.regions = 0)
...
site_buffer <- st_intersects(counties_sf_utm, the_site_counties, sparse = F)
#mapview(site_buffer) does not work?

site_buffer_2 <- counties_sf_utm[the_site_counties,]
mapview(site_buffer_2)
...
huc8s_WGS84 <- st_transform(HUC,c=4326)
huc2_WGS84<- st_transform(HUC2,c=4326)
