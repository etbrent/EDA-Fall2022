...
theURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb' ##changed period=7 to period=24
...
nc_pop <- 
  get_acs(geography = "county",
          ...
          state = "NC",
          geometry = TRUE,
          key="d4ad7874c996c7a83fec0a81466309ddc07d8e22")##key invalid
...
#electricity_df <- data_frame("State"=as.character(the_states),
                             # "Price"=as.numeric(the_price),
                             # "Capacity"=as.numeric(the_capacity),
                             # "Net_Generation"=as.numeric(net_generation),
                             # "Total_Retail"=as.numeric(total_retail))

electricity_df <- data_frame(State=the_states,
                             Price=the_price,
                             Capacity=the_capacity,
                             Net_Generation=net_generation,
                             Total_Retail=total_retail)
electricity_df <- electricity_df[1:51,]
```
...
df_2017_19 <- map(2017:2019, scrape.it) %>%
  bind_rows()
