...
temp_high <- c(91, 93, 72, 78, 85, 82, 78, 75, 73, 70)
temp_low <- c(65, 52, 47, 58, 60, 53, 51, 50, 52,54)
...
temp_C <- function(x){(x-32)*(5/9)}
temp_C(50)

temp_high_C <- temp_C(temp_high)
temp_high_C
temp_high_C_2 <- format(round(temp_high_C, 0), nsmall = 0) ## round to nearest whole number?
temp_high_C_2

temp_low_C <- temp_C(temp_low)
temp_low_C
temp_low_C_2 <- format(round(temp_high_C, 0), nsmall = 0)
temp_low_C_2
...
rownames <- c("09/21", "09/22", "09/23", "09/24", "09/25", "09/26", "09/27", "09/28", "09/29", "09/30")
durham_temps <- data.frame(rownames, temp_high, temp_high_C_2, temp_low, temp_low_C_2)
colnames(durham_temps) <- c("Date", "High (F)", "High (C)", "Low (F)", "Low (C)")
durham_temps
...
summary(durham_temps)
sd(durham_temps$`High (F)`)
lapply(durham_temps, sd)
...
date_obj_past ## year shows up as 2055
...
#############################################################################
#Build a function to fix year that is more general than the one discussed in the lesson
fix.early.dates <- function(str_juneteenth, cutoff) {
       m <- year(str_juneteenth) %% 100  #operator %% is a modular division i.e. integer-divide year(d) by 100 and returns the remainder 
       year(str_juneteenth) <- ifelse(m > cutoff, 1900+m, 2000+m)  #this will update year(d), year() is a function that returns the year for a data object
       return(str_juneteenth)
}
