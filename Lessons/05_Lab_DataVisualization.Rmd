...
PeterPaul_surface <- filter(PeterPaul.chem.nutrients, depth == 0)

PeterPaul_surface_plot <- ggplot(data=PeterPaul_surface, aes(x=daynum, y=temperature_C, color=year4))+
  geom_point()+
  facet_wrap(vars(lakename), nrow=2)+
  ylab("Temperature (\u00B0C)")

PeterPaul_surface_plot
...
PeterPaul_plot2 <- ggplot(data=PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C, color=depth))+
  geom_point(size=0.5)+
...
EPA_air <- ggplot(data=EPAair, aes(x=Ozone, y=PM2.5, color=mean_lat))+
  geom_point(alpha = 0.5)+
  geom_smooth(method=lm, color="black")
  
EPA_air

EPA_air2 <- ggplot(data=EPAair, aes(x=PM2.5, y=Ozone, color=mean_lat))+
  geom_point(alpha = 0.5)+
  geom_smooth(method=lm, color="black")
...
ggplot(data=EPAair, aes(x=Year, y=PM2.5, group=Year))+
  geom_boxplot()
...
#ggplot(data=EPAair, aes(x=Year_num, y=PM2.5, group=Year))+
  #geom_line()


ggplot(data=EPAair, aes(x=as.factor(Month), y=PM2.5))+
  geom_boxplot()+
