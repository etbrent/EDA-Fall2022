...
```{r}
#Exercise 1:
ozone_density <- ggplot(data=EPAair, aes(Ozone, fill=as.factor(Year))) +
  geom_density(alpha=0.25) +
  scale_fill_manual(values=c("red", "blue")) +
  mytheme
ozone_density

ozone_boxplot <- ggplot(data=EPAair, aes(x=as.factor(Year), y=Ozone)) +
  geom_boxplot() +
  stat_summary(geom="text", fun=max, vjust=-1, size=4, label=c("b", "a")) +
  ylim(0, 150)
ozone_boxplot

ozone_violin <- ggplot(data=EPAair, aes(x=as.factor(Year), y=Ozone)) +
  geom_violin(draw_quantiles = 0.5) +
  stat_summary(geom="text", fun=max, vjust=-1, size=4, label=c("b", "a")) +
  ylim(0, 150)
ozone_violin
##draw quantiles already automated for boxplot - shows median? 
...
lm_ozone <- lm(EPAair$PM2.5~EPAair$Ozone)
summary(lm_ozone)
##p value < 2 * 10^-16
##R^2 = 0.13
...
ozone_scatter <- ggplot(data=EPAair, aes(x=Ozone, y=PM2.5)) +
  geom_point() +
  geom_smooth(method=lm) +
  geom_text(x = 130, y = 10, label = "y = 0.38384x + 15.63824", hjust = 1) + 
  geom_text(x = 120, y = 5, label = "SE = 13.06", hjust = 1)
ozone_scatter
...
library(corrplot)
EPAair_2 <- data_frame(EPAair$mean_lat, EPAair$mean_lon,EPAair$Month, EPAair$Year, EPAair$PM2.5, EPAair$Ozone)

EPAair_2 <- drop_na(EPAair_2)

colnames(EPAair_2) <- c("mean_lat", "mean_lon", "Month", "Year", "PM2.5", "Ozone")

EPAair_corr <- cor(EPAair_2)
corrplot(EPAair_corr, method = "ellipse")
corrplot.mixed(EPAair_corr, upper = "ellipse")
...
EPAair_model <- lm(data=EPAair_2, PM2.5 ~ Ozone + mean_lat + mean_lon + Month + Year)
step(EPAair_model)
...
EPAair_model_2 <- lm(data=EPAair_2, PM2.5 ~ Ozone + mean_lat + mean_lon + Month)
summary(EPAair_model_2)
#R-squared = 0.19 (higher R-squared)
...
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = reorder(plotID, treatgroups, na.rm=T), y = dryMass)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5,
               label = c("abc", "ab", "abcd", "bcd", "bcd", "a", 
